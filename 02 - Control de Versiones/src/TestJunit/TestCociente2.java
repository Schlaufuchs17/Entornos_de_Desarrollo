...
Cociente C_1 ; //ACF: Creamos el tipo cociente con el que haremos las pruebas
	...
	public void testDivisionReales() {
		...
		double Obtenido = C_1.divisionReales(70.1, 9.2);
		assertEquals(Esperado, Obtenido);
		
		
		
		Esperado =9.223372036854776E16;
		Obtenido = C_1.divisionReales(70.2, 0);
		assertEquals(Esperado, Obtenido);
		
		
		Esperado =0.0;
		Obtenido = C_1.divisionReales(0, -9870.5);
		assertEquals(Esperado, Obtenido);
	...
	public void testDivisionEnteros() {
		...
		int Esperado = 20;
		double Obtenido = C_1.divisionEnteros(80, 4);
		assertEquals(Esperado, Obtenido);
		
	
		
		Esperado= 0;
		Obtenido = C_1.divisionEnteros(0, 140);
		assertEquals(Esperado, Obtenido);
